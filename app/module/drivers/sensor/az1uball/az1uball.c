...
void az1uball_read_data_work(struct k_work *work)
{
    ...
    if ( (delta_x != 0 || delta_y != 0 )  //
        ...
        if (zmk_keymap_highest_layer_active()<3){  //レイヤーが3未満なら
            ...
            if (delta_y > 10) {
                // 下方向 → ボリュームダウン
                input_report(data->dev, INPUT_KEY_VOLUMEDOWN, 1, true, 0); // 押下
                input_report(data->dev, INPUT_KEY_VOLUMEDOWN, 0, true, 0); // 離す
            } else if (delta_y < -10) {
                // 上方向 → ボリュームアップ
                input_report(data->dev, INPUT_KEY_VOLUMEUP, 1, true, 0);   // 押下
                input_report(data->dev, INPUT_KEY_VOLUMEUP, 0, true, 0);   // 離す
            }
        }
    }
